...
class Component {
    ...
    constructor({width = null, height = null, color = "blue", x = null, y = null, type="", isPlayer = false} = {}, beforeFct){
        ...
        this._keysPressed = [];
        this._type = type;
        if(this._type === "image" || this._type === "background"){
            this._image = new Image();
            this._image.src = this._color;
        }
    ...
    update(){
        ...
        } else if(this._type === "image" || this._type === "background"){
            ctx.drawImage(this._image, this._x, this._y, this._width, this._height);
            if(this._type === "background") ctx.drawImage(this._image, this._x + this._width, this._y, this._width, this._height);
        } else {
            ctx.fillStyle = this._color;
            ctx.fillRect(this._x, this._y, this._width, this._height);
        }
    ...
    updatePos(){
        ...
        if(this._type === "background" && this._x === -this._width) this._x = 0; // Loop of background
    }

    move(movement){
        if(!Array.isArray(movement)) movement = [movement];
        if(this._type === "image") this._image.src = "angry.gif";
    ...
    stopMovement(){
        this._speedX = 0;
        this._speedY = 0;
        if(this._type === "image") this._image.src = "smiley.gif";
    }
